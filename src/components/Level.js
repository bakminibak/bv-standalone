...
export const Level = ({
  ...
  const [nextAudio, setNextAudio] = useState(
    new Audio("./sfx/nextLevel/Positive Game Win.mp3")
  );
  ...
  return (
    <>
      ...
      <div className="level-animation-container" ref={animationContainer}>
        <div className="chests">
          <div className="chests-container">
            <ChestClass
              key="1dsa"
              customClass="k1"
              bonusPoints={levelPrizes}
              numOpenedChest={numOfOpenedChest}
              onClickEl={() => {
                chestClicked();
              }}
            />
        ...
        {isChestOpen && currentLevel < 3 && (
          <button className="btn button_next" onClick={handleBtnClick}>
            <img src="./images/btns/sledeci_nivo.png" />{" "}
          </button>
        )}
        {isChestOpen && currentLevel === 3 && (
          <button className="btn button_getbonus" onClick={handleBtnClick}>
            <img src="./images/btns/PREBACI_BONUS.png" />{" "}
          </button>
        )}
      </div>
    </>
